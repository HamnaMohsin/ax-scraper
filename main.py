...
from scraper4       import extract_aliexpress_product
...
# ── Category scraper schemas ──────────────────────────────────────────────────

class CategoryScrapeRequest(BaseModel):
    categories: list[str] = Field(
        default=DEFAULT_CATEGORIES,
        description="Keywords to search on AliExpress.",
        examples=[["wireless earbuds", "phone case"]],
    )
    pages: int = Field(
        default=MAX_PAGES_PER_CATEGORY,
        ge=1, le=10,
        description="Pages to scrape per category (1–10).",
    )


# ── Category scrape background worker ────────────────────────────────────────

def run_category_scrape_job(job: CategoryScrapeJob) -> None:
    job.status     = JobStatus.RUNNING
    job.started_at = datetime.utcnow()
    print(f"[{job.job_id}] ▶ Category scrape started — {job.categories}")

    results = {}
    try:
        with sync_playwright() as pw:
            for keyword in job.categories:
                if job._cancel:
                    print(f"[{job.job_id}] 🛑 Cancelled before '{keyword}'")
                    job.status      = JobStatus.CANCELLED
                    job.finished_at = datetime.utcnow()
                    return

                print(f"[{job.job_id}] 🔍 '{keyword}'...")
                result      = scrape_category(pw, keyword, job.pages)
                results[keyword] = result
                print(f"[{job.job_id}] ✓ '{keyword}': {result['count']} products")

        total = sum(r["count"] for r in results.values())
        job.results = {
            "scraped_at":          job.started_at.isoformat(),
            "categories_searched": len(results),
            "pages_per_category":  job.pages,
            "total_products":      total,
            "results":             results,
        }
        job.status      = JobStatus.COMPLETED
        job.finished_at = datetime.utcnow()
        print(f"[{job.job_id}] ✅ Done — {total} products")
...
# ── Category scrape endpoints ─────────────────────────────────────────────────

@app.post("/scrape-categories", status_code=202)
def start_category_scrape(req: CategoryScrapeRequest, background_tasks: BackgroundTasks):
    """
    Start an async category scrape job.
    Returns job_id immediately — poll /scrape-categories/{job_id} for status.
    """
    if not req.categories:
        raise HTTPException(status_code=422, detail="categories must not be empty.")

    job_id = str(uuid.uuid4())
    job    = CategoryScrapeJob(job_id=job_id, categories=req.categories, pages=req.pages)
    _category_jobs[job_id] = job

    background_tasks.add_task(run_category_scrape_job, job)
    ...
    return {
        ...
        "pages":      req.pages,
        "message":    "Job queued. Poll /scrape-categories/{job_id} for status.",
    }


@app.get("/scrape-categories/{job_id}")
def get_category_job(job_id: str):
    """Poll job status. Includes results payload when status == completed."""
    job = _category_jobs.get(job_id)
    if not job:
        raise HTTPException(status_code=404, detail=f"Job '{job_id}' not found.")
    return JSONResponse(content=job.to_dict(
        include_results=job.status == JobStatus.COMPLETED
    ))


@app.get("/scrape-categories/{job_id}/results")
def get_category_results(job_id: str):
    """Fetch final results only. Returns 404 if job not completed yet."""
    job = _category_jobs.get(job_id)
    if not job:
        raise HTTPException(status_code=404, detail=f"Job '{job_id}' not found.")
...
@app.get("/scrape-categories")
def list_category_jobs():
    """List all category scrape jobs."""
    return [j.to_dict() for j in _category_jobs.values()]


@app.delete("/scrape-categories/{job_id}")
def cancel_category_job(job_id: str):
    """Cancel a pending/running job (takes effect at next category boundary)."""
    job = _category_jobs.get(job_id)
    if not job:
        raise HTTPException(status_code=404, detail=f"Job '{job_id}' not found.")
