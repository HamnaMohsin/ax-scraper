...
Mirrors the approach of the working main scraper:
  - gatewayAdapt=glo2swe (same as working scraper)
  - Per-attempt `with sync_playwright() as p` block
  - Same is_captcha_page logic (fires on any recaptcha presence)
  - Same rotation pattern (rotate at top of loop on retry)
  - Same 8s render wait after goto before any checks
  - No COOKIES (working scraper doesn't use them)
...
def get_tor_ip() -> str:
    """Verify current Tor exit IP using requests+socks5."""
    try:
...
def rotate_tor_circuit() -> bool:
    """Rotate Tor circuit — same logic as working main scraper."""
    ...
    try:
        ...
        if ip_before == ip_after:
            print("⚠️  WARNING: IP did not change — add more ExitNodes to torrc")
        else:
...
# ── Detection — mirrors working scraper exactly ───────────────────────────────

def is_captcha_page(page) -> bool:
    """
    Detect CAPTCHA/block pages.
    Matches working scraper: fires on ANY matching selector presence (no size gate).
    """
    ...
    # DOM selectors — same list as working scraper
    captcha_selectors = [
...
() => {
    ...
    if (allSkuCols.length > 0) {
        ...
        for (const [rowId, elements] of Object.entries(rowMap)) {
            ...
            // Walk up DOM to find sku-item--property ancestor for label
            let label = null;
            let el    = elements[0];
            ...
            // Fallback: scan ancestor siblings for label text
            if (!label) {
                el = elements[0];
    ...
    // ── Pass 3: debug — return SKU class names found on page ──────────────
    const found = new Set();
...
def scrape_product_variants(product_id: int | str) -> dict:
    pid = int(product_id)
    # Use gatewayAdapt=glo2swe — same as working main scraper
    ...
    for attempt in range(MAX_RETRIES):
        ...
        # Rotate at top of loop on retries — same pattern as working scraper
        if attempt > 0:
        ...
        # Fresh playwright + browser per attempt
        with sync_playwright() as p:
            ...
            try:
                ...
                # Check for CAPTCHA immediately after landing
                if is_captcha_page(page):
                    print("⚠️  CAPTCHA detected — rotating IP and retrying...")
                    browser.close()
                    continue

                # Wait for React to render — same 8s as working scraper
                print("⏳ Waiting for page to render...")
                time.sleep(8)

                # Gentle scroll to trigger lazy variant rendering
                ...
                # Also wait for SKU selector if present (belt-and-suspenders)
                try:
